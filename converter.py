...
def convert_im(path):
    ...
    # Например r, g, b = 130, 126, 123 — это оттенок серого, его максимальная разница 130-123 = 7, что относительно немного
    # А, например, цвет r, g, b = 30, 50, 223 — это оттенок синего, его максимальная разница = 223-30 = 193, что очень много
    ...
    img = ImageOps.invert(Image.fromarray(arr)).resize((28, 28))  # Меняем размер на 28*28 пикселей, как в базе данных
    img = img.convert('L')  # Переводим в чёрно-белое изображение
